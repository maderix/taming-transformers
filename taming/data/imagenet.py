...
class ImageNetBase(Dataset):
    ...
    def _prepare_synset_to_human(self):
        ...
        print("Manjeet:human_dict",self.human_dict)
...
class ImageNetBWBase(Dataset):
    ...
    def __getitem__(self, i):
        ...
        bw = rgb2gray(image)
        bw = gray2rgb(bw)
        #print('manjeet:',np.max(image),np.min(image))
        example["image"] = image
        example["bw"] = bw
        return example
...
class ImageNetBWTrain(ImageNetBWBase):
    NAME = "ILSVRC2012_train"
    URL = "http://www.image-net.org/challenges/LSVRC/2012/"
    AT_HASH = "a306397ccf9c2ead27155983c254227c0fd938e2"
    FILES = [
...
class ImageNetBWValidation(ImageNetBWBase):
    NAME = "ILSVRC2012_validation"
    URL = "http://www.image-net.org/challenges/LSVRC/2012/"
    AT_HASH = "5d6d0df7ed81efd49ca99ea4737e0ae5e3a5f2e5"
    VS_URL = "https://heibox.uni-heidelberg.de/f/3e0f6e9c624e45f2bd73/?dl=1"
...
class BWExamples(Dataset):
    def __init__(self):
        self.preprocessor = get_preprocessor(size=256, additional_targets={"bw": "image"})
        with open("data/bw_examples.txt", "r") as f:
            relpaths = f.read().splitlines()
        self.image_paths = [os.path.join("data/bw_images",
                                         relpath) for relpath in relpaths]
        self.bw_paths = [os.path.join("data/bw_depth",
                                         relpath.replace(".JPEG", ".png")) for relpath in relpaths]
    ...
    def __getitem__(self, i):
        e = dict()
        e["image"] = self.preprocess_image(self.image_paths[i])
        e["bw"] = self.preprocess_bw(self.bw_paths[i])
        transformed = self.preprocessor(image=e["image"], bw=e["bw"])
        e["image"] = transformed["image"]
        e["bw"] = transformed["bw"]
        return e
